...
# exits 2 where GNU grep exits 1).
set -uo pipefail
cd "$(git rev-parse --show-toplevel)" || exit 1

allow_file="${DOT_ENTRIES_ALLOWLIST:-.github/scripts/tracked-dot-entries.allow}"
...
# Every dot-named component, at any depth: a tool directory nested under
# scripts/ or dist/ is the same defect as one at the root.
present=$(printf '%s\n' "$files" | tr '/' '\n' | grep '^\.' | sort -u)
