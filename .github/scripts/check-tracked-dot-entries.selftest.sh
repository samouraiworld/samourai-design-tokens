...
#
# The stand-in for a real tool directory is `.sometool`, deliberately: naming
# an actual one here would publish that name from this public repository,
# which is the thing the check exists to prevent. The check never reads the
# name, only whether the allowlist vouches for it, so a neutral stand-in
# exercises exactly the same path a real one would.
#
...
here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && git rev-parse --show-toplevel)"
check="$here/.github/scripts/check-tracked-dot-entries.sh"
work="$(mktemp -d)"
fails=0
passes=0
...
scaffold() {
  ...
  : > "$d/.gitignore"
  : > "$d/.github/keep"
  printf '%s\n' "$@" > "$d/.github/scripts/tracked-dot-entries.allow"
...
# A tool directory under scripts/ is the same defect. A root-only scan would
# miss it, and a tool that keeps per-workspace state writes exactly there.
d=$(scaffold nested .github .gitignore)
mkdir -p "$d/scripts/.sometool"; : > "$d/scripts/.sometool/settings.json"
git -C "$d" add -A >/dev/null 2>&1
expect_fail_saying "$d" "tracked dot-entry not on the allowlist" IGNORE=1

# --- case 3: a directory below an allowed one --------------------------------
# `.sometool` vouched for at the root must not vouch for `.sometool/.cache`.
# The scan splits on every separator, so each component stands on its own.
d=$(scaffold subdir .github .gitignore .sometool)
mkdir -p "$d/.sometool/.cache"; : > "$d/.sometool/keep"; : > "$d/.sometool/.cache/state"
git -C "$d" add -A >/dev/null 2>&1
expect_fail_saying "$d" "tracked dot-entry not on the allowlist" IGNORE=1
out=$(cd "$d" && bash "$check" 2>&1)
if grep -qF ".cache" <<<"$out" && ! grep -qF "  .sometool" <<<"$out"; then
  passes=$((passes+1)); echo "ok: the nested component is named and the vouched-for parent is not"
else
  echo "FAIL: an allowed parent laundered the component below it"
  printf '%s\n' "$out" | sed 's/^/    /'; fails=$((fails+1))
fi

# --- case 4: a stale allowlist line ------------------------------------------
...
rm "$d/.github/scripts/tracked-dot-entries.allow"
expect_fail_saying "$d" "cannot read the allowlist" IGNORE=1

# --- case 6: the allowlist is emptied ----------------------------------------
