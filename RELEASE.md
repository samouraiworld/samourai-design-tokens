...
4. **The decider is named.** Every pull request since the previous tag that changes what a consumer renders names, in its description, who decided it. The trigger is the generated output, not the file edited: a value in `tokens.json`, and equally a design constant the build scripts emit into `dist/` — `scripts/build.mjs` holds two today, `SPEC.pageGradient` and `SPEC.focusRing` (the latter emitted as `--focus-ring` and the preset's `ringWidth.DEFAULT` / `ringColor.DEFAULT`), and they are examples rather than the list — the rule is written against the output so that the next such constant is covered without rewriting this step. A token value change is a design decision (AGENTS.md, "The source of truth"), and a build constant that reaches `dist/` is one on the same footing: a consumer cannot tell which file a rendered value came from, and a tag that carries either unsigned ships a value nobody chose. No name, no tag.
